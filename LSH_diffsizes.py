...
for subset_size in subset_sizes:
    ...
    subset_indices = np.random.choice(len(test_features), subset_size, replace=False)
    subset_test_features = test_features[subset_indices]
    subset_test_img_paths = [test_img_paths[idx] for idx in subset_indices]

    # Measure the query times for multiple queries
    total_query_time = 0
    for _ in range(num_queries):
        query_index = np.random.choice(len(subset_test_features))  # Randomly choose a query index
        query_vec = subset_test_features[query_index]  # Use the query vector from the test subset
        ...
        max_search_radius = 2
        all_neighbors = lsh.query(query_vec, len(subset_test_features), max_search_radius)
