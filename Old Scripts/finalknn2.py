...
plt.savefig('Aplotsknn/query88FT.pdf')
...
plt.savefig('Aplotsknn/distributionft.pdf')
...
np.save(os.path.join(save_dir, "dataset_sizes_KNN_FT3.npy"), np.array(dataset_sizes))
np.save(os.path.join(save_dir, "query_times_KNN_FT3.npy"), np.array(query_times))
...
nbrs = NearestNeighbors(metric='cosine', algorithm='brute', radius=0.004).fit(test_features)
