...
subset_sizes = np.linspace(1000, train_features_length, 20, dtype=int)

# Number of runs to average for each subset size
num_runs = 10

# Lists to store dataset sizes and memory usage
dataset_sizes = []
memory_usages = []
...
for subset_size in subset_sizes:
    ...
    for _ in range(num_runs):
        ...
        # Calculate the memory usage of LSH
        lsh_memory_usage = memory_after - memory_before
        total_memory_usage += lsh_memory_usage
    
    # Calculate the average memory usage for this subset size
    average_memory_usage = total_memory_usage / num_runs
    
    # Save the dataset size and memory usage
    dataset_sizes.append(subset_size)
    memory_usages.append(average_memory_usage)
    
    # Save the LSH object for this subset size
    lsh_filename = f"lsh_subset_{subset_size}.npy"
    np.save(lsh_filename, lsh)

    print(f"Subset Size: {subset_size}, Average Memory Usage: {average_memory_usage / (1024 * 1024):.2f} MB")

# Save arrays to .npy files
np.save(os.path.join(save_dir, "dataset_sizes_memory.npy"), np.array(dataset_sizes))
np.save(os.path.join(save_dir, "memory_usages.npy"), np.array(memory_usages))