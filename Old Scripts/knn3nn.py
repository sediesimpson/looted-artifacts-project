...
nbrs = NearestNeighbors(metric='cosine', algorithm='brute', radius=0.01).fit(train_features)

# Get distances and indices for a single query image
query_index = 70 # Change this to select a different query image
...
plt.savefig('Aplotsknn/query70.png')
...
plt.legend()
plt.savefig('Aplotsknn/precisionrecallNN2.pdf')
...
plt.savefig('Aplotsknn/prcurve2.pdf')
...
# Find the optimal radius
#------------------------------------------------------------------------------------------------------------------------
radii = np.linspace(0.001, 0.1, 100)
...
for radius in radii:
    ...
    for query_index in range(len(train_features)):
        ...
        non = len(indices) # how many neighbors are returned 
...
np.save('Frecalls/recalls50.npy', recalls)
np.save('Fprecisions/precisions50.npy', precisions)
np.save('Ff1scores/f1scores50.npy', f1_scores)